...
class BackupHistoryScreenController(Screen):
    ...
    def go_to_main_menu(self):
        """Volta para o menu principal (options screen)"""
        try:
            self._clear_all_screens()
            
            if self.manager and self.manager.has_screen('options_screen'):
                self.manager.current = 'options_screen'
            else:
                print("Tela 'options_screen' não encontrada no ScreenManager.")
        except Exception as e:
            print(f"Erro ao navegar para o menu principal: {e}")
