...
class TransferSoftwareScreen(Screen):
    ...
    def load_software_list(self):
        ...
        software_dir = r"C:\Users\gabri\Downloads\Softwares"
    ...
    def _finalize_without_software(self):
        ...
        # Obtém a sessão de backup
        backup_session = get_backup_session()
        
        # Define o fim do backup se ainda não foi definido
        if not backup_session.end_time:
            backup_session.end_time = datetime.now()
        
        # Calcula duração total
        if backup_session.start_time:
            duration = (backup_session.end_time - backup_session.start_time).total_seconds()
        else:
            duration = 0
        
        # Obtém resumo da sessão (apenas arquivos, sem software)
        summary = backup_session.get_summary()
        
        print("=" * 50)
        print("FINALIZAÇÃO DO BACKUP (APENAS ARQUIVOS):")
        print(f"  - Usuário: {summary['user_name']} (ID: {summary['user_id']})")
    ...
    def copy_software(self, selected_software, destination_path):
        ...
        backup_session.set_software_backup_data(
            files=copied_files,
            size=total_size,
        ...
        # Navega para a tela de histórico
        if self.manager.has_screen('backup_history_screen'):
            backup_history_screen = self.manager.get_screen('backup_history_screen')
            self.manager.current = 'backup_history_screen'
        else:
            print("Tela 'backup_history_screen' não encontrada!")